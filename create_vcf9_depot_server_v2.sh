...
configure_nginx() {
  ...
  cat > "${NGINX_CONF}" <<EOF
server {
    listen ${DEPOT_HTTP_PORT} default_server;
    ...
    # Serve the official depot structure at /PROD/ as fallback
    location /PROD/ {
...
configure_firewall() {
  ...
  if command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active --quiet firewalld; then
    info "Opening firewall for TCP/${DEPOT_PORT} (HTTPS) and TCP/${DEPOT_HTTP_PORT} (HTTP)"
...
print_summary() {
  local depot_url_https="https://${DEPOT_FQDN}:${DEPOT_PORT}/${DEPOT_NAME}/PROD"
  local depot_url_http="http://${DEPOT_FQDN}:${DEPOT_HTTP_PORT}/${DEPOT_NAME}/PROD"
  cat <<EOF

[DONE] VCF9 depot server (v${SCRIPT_VERSION}) is ready.

Depot URL:
  HTTPS: ${depot_url_https}
  HTTP:  ${depot_url_http}
